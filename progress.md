...
- Implementada a camada de áudio local por acionamento manual: Flutter grava WAV temporário, FastAPI/Vosk transcreve em português e o texto segue ao fluxo existente de comandos. O microfone analógico do notebook foi identificado pelo PipeWire; `parecord`, `pactl` e `ffmpeg` já estão disponíveis.
- Validado o modelo local compacto `vosk-model-small-pt-0.3` (52 MB descompactado, fora do Git): WAV silencioso retornou `422 no_speech_recognized`, sem criar comando. Testes Python passaram em 16 casos; análise, teste e build Linux do Flutter passaram.
...
- Substituída a voz padrão mecânica pela voz neural local Piper `pt_BR-faber-medium` (61 MB em `models/piper/`, ignorado pelo Git). A síntese e a reprodução foram validadas em WAV 22,05 kHz; `spd-say` permanece como reserva automática se o Piper não estiver disponível.
- Implementada a confirmação conversacional do comando por voz: após cada transcrição, o Jubileu a lê e aguarda `confirmo`/`enviar` ou `cancelo`/`descartar` em uma nova gravação. Criações continuam recebendo uma segunda confirmação formal do backend, vinculada ao contexto opaco persistido.
