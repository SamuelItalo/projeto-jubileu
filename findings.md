...
- Para melhorar a naturalidade, Piper `1.7.0` foi instalado no ambiente isolado `.venv`. A voz escolhida é `pt_BR-faber-medium`: um único locutor, qualidade média, 22,05 kHz e conjunto de dados CC0; o modelo ocupa cerca de 61 MB em `models/piper/` e não é versionado. A execução ocorre em CPU, sem depender da GPU.
- O Ollama `0.32.3` com `qwen3:4b-instruct` foi validado localmente na RTX 2050. A API Docker o acessa pelo gateway do host; o serviço precisa escutar na porta `11434`. O modelo só recebe transcrições fora da gramática e sua saída JSON é revalidada antes de qualquer confirmação ou alteração.
- Para privilegiar frases naturais ditadas, o modo `ollama_first` consulta o modelo antes do parser somente quando a origem é `voice`; comandos digitados conservam a resposta imediata do parser determinístico.
- O Vosk compacto privilegia baixo consumo, mas exige dicção mais lenta e perde termos em fala natural. Whisper `small` em CPU com `int8` é a nova escolha local de precisão; o custo esperado é alguns segundos de processamento após o fim da gravação.
- A confirmação local tolera variações que iniciam por `confirm` ou `conferm`, reduzindo impacto de pequenas trocas do Whisper. A conclusão explícita da tarefa atualmente ativa foi autorizada sem confirmação intermediária; criações continuam exigindo as duas confirmações.
